...
(async function ($, Swal10, PLUGIN_ID) {
  ...
  async function convertDateJapan(eraInput) {
    if (window.BoK) {
      if (!window.BoK.Constant.JpCalenderBase) {
        Swal10.fire({
          position: "center",
          icon: "error",
          text: "Use the Constant Control plugin",
    ...
    // Parse input based on supported formats
    ...
    // Convert parsed strings to integers for calculations
    ...
    // Find the era data matching the provided symbol (case-insensitive)
  ...
  async function inputDate(input) {
    ...
    try {
      ...
      if (/^\d{8}$/.test(input)) {
        ...
        //Check format of Japan Era //eYYMMDD, e YY MM DD, e Y MM D
        let revert = await convertDateJapan(input);
  ...
  async function getFormat(dateValue, format) {
    if (!dateValue) return "";
  ...
  kintone.events.on(
    ...
    async (event) => {
      ...
      for (let item of CONFIG.formatSetting) {
        ...
        console.log(item.storeField.code);
        ...
        const defaultInputValue = await getFormat(defaultDate, item.format);
        ...
        const input = $("<div>").append(
          $("<input>")
            ...
            .on("change", async (e) => {
              const changeFormat = await inputDate(e.target.value.trim());
              if (changeFormat === false) {
                ...
                $(inputError).hide();
                e.target.value = await getFormat(changeFormat, item.format);
  ...
  kintone.events.on("app.record.detail.show", async (event) => {
    ...
    for (const item of CONFIG.formatSetting) {
      ...
      const formatDate = await getFormat(dateValue, item.format);
      if (formatDate === false) {
        errorMessage = "Please use the Constant Management plugin.";
      } else {
      ...
      createInputDate.forEach((item) => {
        ...
        if (getFieldResponse.properties[field]) {
                  ...
                  if (currentInputType !== "exact") {
                    ...
                    $(dateInput)
                      ...
                      .on("change", async function (e) {
                        let changeFormat = await inputDate(
                          e.target.value.trim()
                        );
                        if (changeFormat === false) {
                          inputError.show();
          ...
          dateInput.find("input").on("change", async function (e) {
            let changeFormat = await inputDate(e.target.value.trim());
            if (changeFormat === false) {
              inputError.show();
            } else {
              inputError.hide();
      ...
      for (const item of CONFIG.searchContent) {
        ...
        if (nextElement.length) {
          if (nextElement.hasClass("exact")) {
            ...
            let changFormat = await inputDate(value.trim());
            searchInfoList.push({
              fieldSearch: {
            ...
            let changFormatStart = await inputDate(startDate.trim());
            let endDate = nextElement.find("input").last().val();
            let changFormatEnd = await inputDate(endDate.trim());
            searchInfoList.push({
              fieldSearch: {
                code: item.fieldSearch.code,
                label: item.fieldSearch.label,
